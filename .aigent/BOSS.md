...
- **Comandos atómicos, no compuestos.** Ejecuta **un comando simple por llamada** de shell. **No** encadenes con `cd "<ruta>" && …`: trabaja desde la **raíz del proyecto** (donde viven `.aigent/` y `.context/`) y usa rutas relativas; si necesitas otra carpeta, pásala como argumento del propio comando, no con un `cd` previo. **Evita pipes y redirecciones** (`| sed`, `| head`, `2>/dev/null`) cuando un solo comando o las tools de lectura nativas bastan. **Invoca skills y scripts siempre por el launcher** `.aigent/IDE/bin/run …` (prefijo estable), nunca `node …` a secas (§12.7-bis). Motivo: el permiso de Claude Code casa por **prefijo de comando** y, por seguridad, **no aprueba comandos compuestos** aunque cada verbo esté permitido — un encadenado obliga a aprobar el literal completo una y otra vez. Comandos atómicos = matchean `verbo:*` = cero fricción de permisos. (La herramienta Bash es sin estado: por eso conviene la raíz como cwd en vez de `cd` repetidos.)
