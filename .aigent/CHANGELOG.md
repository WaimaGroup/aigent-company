...
- **No descarga si ya hay un runtime usable.** Antes de bajar nada comprueba los dos sitios que el launcher resuelve — el bundled ya descargado en `deps/` y un Node del sistema en `PATH` — y si cualquiera es ≥20, se salta la descarga. Solo baja el bundled si no hay ninguno. `--node-install --force` / `-NodeInstall -Force` fuerza la descarga de la versión fijada aunque ya exista otro Node (reproducibilidad / offline). En instalación completa el runtime se asegura con esta misma lógica.
