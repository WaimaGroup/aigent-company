...
3. Una vez confirmado, las lecturas de contexto van a `.context/<proyecto>/finance/` y los entregables van a la ruta indicada en `config.json del proyecto → paths.finance` (ver Paso 0.5).

> El bootstrap de BOSS.md garantiza que `.context/`, `config.json` y `.context/<proyecto>/` existen. Si `.context/<proyecto>/finance/` no existe, créala con `prd.md` y `tasks.md` vacíos desde las plantillas.
